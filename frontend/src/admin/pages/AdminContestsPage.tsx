    ...
    <div style={{ padding: "32px 36px" }}>
      ...
      {showForm && (
        <div style={{ background: "#141414", border: "1px solid #2A2A2A", borderRadius: 12, padding: 24, marginBottom: 24 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: "#ddd", marginBottom: 16 }}>New Contest</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div style={{ gridColumn: "span 2" }}>
              <label style={LABEL_STYLE}>Match ID * <span style={{ color: "#444", fontWeight: 400 }}>(copy from Matches page)</span></label>
              <input value={form.matchId} onChange={(e) => setF("matchId", e.target.value)} placeholder="ObjectId..." style={INPUT_STYLE} />
            </div>
            <div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #1E1E1E" }}>
                {["Name", "Type", "Entry", "Prize Pool", "Spots", "Status", "ID"].map((h) => (
                  <th key={h} style={{ padding: "12px 16px", textAlign: "left", fontSize: 12, fontWeight: 700, color: "#444", textTransform: "uppercase" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {contests.length === 0 && <tr><td colSpan={7} style={{ padding: 24, textAlign: "center", color: "#444" }}>No contests yet</td></tr>}
              {contests.map((c) => (
